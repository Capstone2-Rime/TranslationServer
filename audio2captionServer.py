...
try:
    ...
    def note2client(client_list):
        ...
        while True:
            ...
            if(isNote):
                if(len(dataset)>0):
                    item=bj.bojung(dataset,loc,item)

            else:
                if(sessionType != None):
                    #print("note2client-sessionType :",sessionType)
                    item=bj.bojung_csv(sessionType,loc,item)
    ...
    def make_bojung_data(client_list):
        ...
        if(isNote):
            ...
            fff = open(loc+'afterset.txt','r', encoding='utf-8')
            line=fff.read()
            data_ls=line.split(',')
            for i in data_ls:
            ...
            if(sessionType != None):
                #print("make bojung data - type :",sessionType)
                loc="/home/ubuntu/workspace/demo/dataset/"#"/home/ubuntu/workspace/demo/dataset/example"+str(sessionType)+".csv"
    ...
    global stop2
    global stop3
    stop=True # course note flag
    stop2=False # session type flag
    stop3=False # file name flag
